...
await startLocalProofServer({ directory: process.env.PROOF_LOCAL_DIRECTORY ?? "", secret: process.env.PROOF_LOCAL_SECRET ?? "",
  origin: process.env.PROOF_WEB_ORIGIN ?? "http://localhost:3000", port: Number(process.env.PROOF_LOCAL_PORT ?? "3736") });
