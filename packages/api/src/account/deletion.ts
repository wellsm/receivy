...
export async function eraseAccount(db: DbClient, userId: string, confirmation: string): Promise<{ deleted: boolean }> {
  ...
  return db.transaction(async tx => {
    ...
    for (const rule of rules.records) {
      await tx.recurrences.updateOne({ where: { id: rule.id }, data: { state: "ended", description: "Registro de conta excluída", ...{ payment_method_id: sqlNull }, updated_at: now } });
      await tx.recurrence_reminders.deleteMany({ where: { recurrence_id: rule.id } });
    }
    const charges = await tx.charges.findMany({ select: { id: true, creditor_id: true, recipient_user_id: true, recipient_email_snapshot: true }, where: { OR: [{ creditor_id: userId }, { recipient_user_id: userId }, { recipient_email_snapshot: user.email }] }, lock: true });
    ...
    const affected = new Set([...charges.records.map(x => x.id), ...ownedProofs.records.map(x => x.charge_id), ...ownedIntents.records.map(x => x.charge_id)]);
    for (const chargeId of [...affected].sort()) {
      const charge = await tx.charges.findOne({ select: { id: true, creditor_id: true, recipient_user_id: true, recipient_email_snapshot: true }, where: { id: chargeId }, lock: true });
      if (!charge) continue;
      ...
      if (recipientDeleted || creditorDeleted) {
        await tx.charges.updateOne({ where: { id: chargeId }, data: {
          ...(recipientDeleted ? { recipient_user_id: sqlNull, recipient_email_snapshot: sqlNull, recipient_name_snapshot: "Conta excluída" } : {}),
          ...(creditorDeleted ? { pix_key_snapshot: sqlNull, pix_key_type_snapshot: sqlNull, pix_label_snapshot: sqlNull } : {}), updated_at: now } });
        ...
        await tx.outbox_events.updateMany({ where: { aggregate_id: chargeId }, data: { state: "failed", ...{ recipient_user_id: sqlNull }, recipient_email: sqlNull, payload: "{}", updated_at: now } });
        await tx.notification_deliveries.updateMany({ where: { charge_id: chargeId, state: "pending" }, data: { state: "suppressed", reason: "account_deleted", updated_at: now } });
        await tx.notification_deliveries.updateMany({ where: { charge_id: chargeId }, data: { render_inputs: "{}", recipient_key: "deleted", recipient_user_id: sqlNull, updated_at: now } });
      }
      const proofs = await tx.payment_proofs.findMany({ select: { id: true, object_key: true }, where: { charge_id: chargeId, sender_user_id: userId } });
      ...
      const intents = await tx.upload_intents.findMany({ select: { id: true, object_key: true }, where: { charge_id: chargeId, sender_user_id: userId } });
      ...
      await tx.payment_proofs.updateMany({ where: { charge_id: chargeId, reviewer_id: userId }, data: { ...{ reviewer_id: sqlNull }, reason: sqlNull } });
    }
    const people = await tx.people.findMany({ select: { id: true, owner_id: true }, where: { OR: [{ owner_id: userId }, { linked_user_id: userId }, { active_email: user.email }] } });
    for (const person of people.records) {
      await tx.person_contacts.deleteMany({ where: { person_id: person.id } });
      const referenced = await tx.charges.count({ where: { debtor_person_id: person.id } }) + await tx.expense_allocations.count({ where: { person_id: person.id } }) + await tx.recurrence_allocations.count({ where: { person_id: person.id } });
      if (person.owner_id === userId && !referenced) await tx.people.deleteOne({ where: { id: person.id } });
      else await tx.people.updateOne({ where: { id: person.id }, data: { name: "Conta excluída", active_email: sqlNull, ...{ linked_user_id: sqlNull }, archived_at: now, updated_at: now } });
    }
    ...
    for (const expense of expenses.records) if (!await tx.charges.count({ where: { source_id: expense.id } })) {
