...
export async function assertActiveSession(db: DbClient, identity: SessionIdentity) {
  const family = await db.session_families.findOne({ select: { id: true }, where: { id: identity.familyId, user_id: identity.userId, revoked_at: { isNull: true } } });
...
export async function disableSessionDevices(tx: DbClient, userId: string, familyId?: string) {
  const devices = await tx.device_tokens.findMany({ select: { id: true }, where: { user_id: userId,
    ...(familyId ? { OR: [{ session_family_id: familyId }, { session_family_id: { isNull: true } }] } : {}) } });
  const now = new Date().toISOString();
  for (const device of devices.records) {
    await tx.notification_deliveries.updateMany({ where: { device_id: device.id, state: "pending" }, data: { state: "suppressed", reason: "session_revoked", render_inputs: "{}", updated_at: now } });
...
export async function listSessions(db: DbClient, userId: string, currentId: string) {
  const rows = await db.session_families.findMany({ select: { id: true, device_name: true, created_at: true, last_seen_at: true }, where: { user_id: userId, revoked_at: { isNull: true } } });
  return rows.records.map(row => ({ id: row.id, deviceName: row.device_name ?? "Dispositivo", createdAt: row.created_at, lastSeenAt: row.last_seen_at, current: row.id === currentId }));
}
...
export async function revokeSession(db: DbClient, userId: string, familyId: string) {
  await db.transaction(async tx => {
    ...
    await tx.activity_events.insertOne({ data: { id: crypto.randomUUID(), subject_user: { id: userId }, actor_user: { id: userId }, type: "account.session_revoked", aggregate_type: "session", aggregate_id: familyId, payload: "{}", created_at: now } });
  });
}
