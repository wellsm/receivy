...
export function appleKeyAvailable(encoded: string): boolean {
  try {
    appleEncryptionKey(encoded);
    return true;
  } catch {
    return false;
  }
...
export function appleFingerprint(key: string, clientId: string, subject: string): string {
  return createHmac('sha256', Buffer.from(hkdfSync('sha256', appleEncryptionKey(key), 'receivy-apple-v1', 'subject-fingerprint', 32)))
...
export function encryptAppleToken(key: string, id: string, clientId: string, token: string): string {
  ...
  const nonce = randomBytes(12),
    cipher = createCipheriv('aes-256-gcm', appleEncryptionKey(key), nonce);
...
export function decryptAppleToken(key: string, row: { id: string; client_id: string; ciphertext?: string }): string {
  ...
  cipher.setAAD(Buffer.from(JSON.stringify([row.id, row.client_id])));
...
export async function journalAppleCredential(
  db: DbClient,
  key: string,
  clientId: string,
  token: string,
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.apple_credentials.insertOne({
      data: {
        id,
        client_id: clientId,
        fingerprint: 'unknown',
        ciphertext,
        state: 'pending',
        reason: 'exchange_staged',
        attempts: 0,
        available_at: new Date(now + STAGING_MS).toISOString(),
        created_at: stamp,
        updated_at: stamp
      }
    });
...
export async function bindAppleCredential(db: DbClient, key: string, id: string, subject: string): Promise<void> {
  await db.transaction(async (tx) => {
    await lockAccountReferences(tx, 'write');
    const row = await tx.apple_credentials.findOne({
      select: { client_id: true, state: true, attempts: true, reason: true },
      where: { id },
      lock: true
    });
    if (!row || row.state !== 'pending' || row.attempts || row.reason !== 'exchange_staged') throw new HttpConflictError();
    await tx.apple_credentials.updateOne({
      where: { id },
      data: { fingerprint: appleFingerprint(key, row.client_id, subject), updated_at: new Date().toISOString() }
    });
  });
}
...
export async function claimAppleActivation(db: DbClient, id: string, now = Date.now()): Promise<void> {
  ...
  const row = await db.apple_credentials.findOne({
    select: { fingerprint: true, client_id: true, state: true, reason: true, attempts: true, available_at: true },
    where: { id },
    lock: true
  });
  if (!row || row.state !== 'pending' || row.reason !== 'exchange_staged' || row.attempts || Date.parse(row.available_at) <= now)
    throw new HttpConflictError();
  const blocking = await db.apple_credentials.count({
    where: {
      id: { not: id },
      state: { isIn: ['pending', 'revoking', 'blocked'] },
      OR: [{ fingerprint: row.fingerprint }, { fingerprint: 'unknown', client_id: row.client_id }]
    }
  });
...
export async function activateAppleCredential(db: DbClient, id: string, userId: string): Promise<void> {
  await db.apple_credentials.updateOne({
    where: { id },
    data: { user: { id: userId }, state: 'active', reason: 'retained_for_account_deletion', updated_at: new Date().toISOString() }
  });
}
...
export async function detachAppleCredentials(db: DbClient, userId: string, now: string): Promise<ProviderRevocation> {
  ...
  if (count) {
    await db.apple_credentials.updateMany({
      where: { user_id: userId },
      data: { ...{ user_id: sqlNull }, state: 'pending', reason: 'account_deleted', available_at: now, updated_at: now }
    });
    return 'pending';
  }
  if (!hasIdentity) return 'not_required';
  await db.apple_credentials.insertOne({
    data: {
      id: randomUUID(),
...
export async function drainAppleRevocations(db: DbClient, key: string, revoke: AppleRevoke, clock = Date.now) {
  const candidates = await db.apple_credentials.findMany({
    select: { id: true, fingerprint: true },
    where: {
      state: { isIn: ['pending', 'revoking', 'blocked'] },
      reason: { not: 'blocked_missing_credential' },
  ...
  for (const candidate of candidates.records) {
    const claim = await db.transaction(async (tx) => {
      await lockAccountReferences(tx, 'write');
      await lockFingerprint(tx, candidate.fingerprint);
      const row = await tx.apple_credentials.findOne({
        select: {
          id: true,
          client_id: true,
      ...
      if (
        !row ||
        !['pending', 'revoking', 'blocked'].includes(row.state) ||
      ...
      let token: string;
      try {
        token = decryptAppleToken(key, row);
      } catch {
        await tx.apple_credentials.updateOne({
          where: { id: row.id },
          data: {
            ...
            available_at: new Date(now + 3600_000).toISOString(),
            updated_at: stamp
          }
        });
        blocked++;
      ...
      const lease = new Date(now + LEASE_MS).toISOString(),
        attempts = Math.min(row.attempts + 1, 1_000_000);
      await tx.apple_credentials.updateOne({
        where: { id: row.id },
        data: { state: 'revoking', lease_until: lease, available_at: lease, attempts, updated_at: stamp }
      });
    ...
    try {
      result = await revoke({ token: claim.token, clientId: claim.row.client_id });
    } catch {
      result = 'transient';
    }
    await db.transaction(async (tx) => {
      await lockAccountReferences(tx, 'write');
      await lockFingerprint(tx, claim.row.fingerprint);
      const row = await tx.apple_credentials.findOne({
        select: { state: true, lease_until: true },
      ...
      if (result === 'revoked') {
        await tx.apple_credentials.updateOne({
          where: { id: claim.row.id },
          data: { state: 'revoked', reason: 'provider_confirmed', ciphertext: sqlNull, lease_until: sqlNull, updated_at: stamp }
        });
        revoked++;
      } else {
        await tx.apple_credentials.updateOne({
          where: { id: claim.row.id },
          data: {
            state: result === 'configuration_error' ? 'blocked' : 'pending',
            reason: result,
            lease_until: sqlNull,
            available_at: new Date(now + Math.min(24 * 3600_000, 60_000 * 2 ** Math.min(claim.attempts, 11))).toISOString(),
            updated_at: stamp
          }
        });
