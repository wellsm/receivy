...
export async function beginNativeApple(db: DbClient, clientChallenge: string) {
  ...
  await createAuthRepository(db).createAttempt({ clientChallenge, stateHash: hashOauthValue(values.state), nonce: values.nonce, codeVerifier: values.codeVerifier, provider: "apple", destination: DESTINATION, expiresAt: new Date(Date.now() + 10 * 60_000) });
  return { state: values.state, nonce: values.nonce };
}
export async function exchangeNativeApple(db: DbClient, input: { state: string; authorizationCode: string; codeVerifier: string; profile?: string; deviceName?: string }, client: OauthProviderClient, secret: string): Promise<AuthSessionResponse> {
  ...
  const attempt = await db.transaction(async tx => {
    ...
    if (!row || row.destination !== DESTINATION || row.clientChallenge !== hashOauthValue(input.codeVerifier)) throw new HttpUnauthorizedError();
    return row;
  });
  // Provider code exchange is deliberately outside the account/row locks.
  const identity = await client.verifyAuthorizationCode({ code: input.authorizationCode, codeVerifier: attempt.codeVerifier, nonce: attempt.nonce, profile: input.profile });
  return db.transaction(async tx => {
    ...
    const repo = createAuthRepository(tx), user = await repo.resolveUser({ provider: "apple", identity });
    ...
    return { accessToken: issueAccessToken({ familyId: session.familyId, secret, userId: user.id }), refreshToken: session.refreshToken, expiresIn: 900, user };
  });
}
