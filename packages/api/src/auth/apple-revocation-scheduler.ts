...
export async function appleRevocationJobHandler(_request: Cron.Incoming<null>, context: Service.Context<AppleRevocationScheduler>): Promise<void> {
  const result = await drainAppleRevocations(context.db, context.variables.APPLE_CREDENTIAL_ENCRYPTION_KEY_B64, createAppleRevoker(decodeOauthProviderConfig(context.variables.OAUTH_PROVIDERS_CONFIG_B64)));
