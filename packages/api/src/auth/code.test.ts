...
import { canAttemptEmailCode, createEmailCodeHash, generateEmailCode, verifyEmailCodeHash } from './code';
...
describe('email login code security', () => {
  ...
  it('rejects consumed, expired, or exhausted records', () => {
    ...
    expect(canAttemptEmailCode({ ...active, expiresAt: new Date('2026-09-04T18:00:00.000Z') }, now)).toBe(false);
    expect(canAttemptEmailCode({ ...active, consumedAt: new Date('2026-09-04T17:59:00.000Z') }, now)).toBe(false);
  });
});
