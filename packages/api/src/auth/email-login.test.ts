...
describe("passwordless email login", () => {
  it("normalizes the address, replaces older codes and delivers only accepted codes", async () => {
    ...
    await requestEmailCode(
      { email: " Ana@Example.COM " },
      { codeHashKey: "hash-key", repo, transport, generateCode: () => "123456" },
    );

    expect(repo.replaceLoginCode).toHaveBeenCalledWith(expect.objectContaining({
      email: "ana@example.com",
      code: "123456",
