...
describe("OAuth flow", () => {
  it("rejects a consumed or unknown state before contacting the provider", async () => {
    ...
    await expect(completeOauth({ code: "provider-code", provider: "google", state: "replayed" }, {
      providerClient, repo,
  ...
  it("returns cancellation only to the destination of a consumed valid state", async () => {
    ...
    vi.mocked(repo.consumeAttempt).mockResolvedValue({
      codeVerifier: "v", clientChallenge: "c", nonce: "n", destination: "receivy://auth/callback",
    });
    const providerClient = { authorizationUrl: vi.fn(), verifyAuthorizationCode: vi.fn() };
    expect(await completeOauth({ error: "access_denied", provider: "apple", state: "valid" }, {
      providerClient, repo,
  ...
  it("persists only a hash of opaque state and returns a provider URL", async () => {
    ...
    expect(repo.createAttempt).toHaveBeenCalledWith(expect.objectContaining({
      codeVerifier: "verifier",
      nonce: "nonce",
  ...
  it("returns only a short one-time Receivy grant after provider verification", async () => {
    ...
    const result = await completeOauth({ code: "provider-code", provider: "google", state: "state" }, {
    ...
    expect(repo.createGrant).toHaveBeenCalledWith(expect.objectContaining({
  ...
  it("exchanges a grant once and issues the normal Receivy session", async () => {
    ...
    const session = await exchangeOauthGrant({ code: "receivy-grant", codeVerifier: "v".repeat(43), deviceName: "iPhone" }, {
    ...
    await expect(exchangeOauthGrant({ code: "bad", codeVerifier: "v".repeat(43) }, {
      accessTokenSecret: "test-only-secret-at-least-32-characters",
