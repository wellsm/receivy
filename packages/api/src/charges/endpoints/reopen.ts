...
export async function reopenChargeHandler(request: IdRequest, { db }: Service.Context<ChargeProvider>): Promise<ItemResponse> {
  return { status: 200, body: await ChargeRepository.reopen(db, request.identity.userId, request.parameters.id) };
}
