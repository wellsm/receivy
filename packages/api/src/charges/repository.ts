...
async function paymentFor(db: DbClient, chargeId: string): Promise<PaymentRecord | null> {
  const row = await db.payments.findOne({
    select: { id: true, charge_id: true, amount_cents: true, currency: true, method: true, paid_at: true, created_at: true },
...
export async function chargeDto(db: DbClient, row: ChargeRow, direction: 'receivable' | 'payable'): Promise<ChargeDetail> {
  return {
    ...
    sharingState:
      row.state !== 'pending'
        ? 'closed'
        : row.pix_key_snapshot && row.pix_key_type_snapshot
          ? 'ready'
          : (await db.public_links.count({ where: { charge_id: row.id } }))
            ? 'legacy_without_pix'
            : 'pix_required',
    pix:
      row.pix_key_type_snapshot && row.pix_key_snapshot
        ...
        : null,
    payment: await paymentFor(db, row.id),
  ...
  for (const subjectId of subjects)
    await db.activity_events.insertOne({
      ...
      data: {
        ...
        actor_user: { id: input.actorId },
        subject_user: { id: subjectId },
        type: input.type,
        aggregate_type: 'charge',
        aggregate_id: input.row.id,
        payload: '{}',
...
export async function recordManualPayment(
  ...
  creditorId: string,
  id: string,
  input: { method: PaymentRecord['method']; paidAt?: string }
