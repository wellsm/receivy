...
import { type ExpenseDetail, type ExpenseInput, type ExpensePlan, type PaymentMethod, planExpenseCharges } from '@receivy/common';
...
export async function persistChargePlan(
  ...
  const notificationSchedule =
    source.source === 'expense'
      ? { offsets: (await getPreferences(db, ownerId)).reminderOffsets, timezone: owner?.timezone ?? 'America/Sao_Paulo' }
      : undefined;
  for (const item of plan.charges) {
    ...
    const row = await db.charges.insertOne({
      ...
      data: {
        ...
        ...(source.occurrenceId ? { source_occurrence_id: source.occurrenceId } : {}),
    ...
    if (recipient.linkedUserId)
      await db.activity_events.insertOne({
        ...
        data: {
          id: crypto.randomUUID(),
          actor_user: { id: ownerId },
          subject_user: { id: recipient.linkedUserId },
          type: 'charge.created',
          aggregate_type: 'charge',
          aggregate_id: row.id,
          payload,
    ...
    await db.outbox_events.insertOne({
      ...
      data: {
        ...
        payload: JSON.stringify({ chargeId: row.id, source: row.source, notificationSchedule }),
        state: 'pending',
        attempts: 0,
        available_at: now,
        created_at: now,
...
async function expenseDto(
  ...
  const allocations = await db.expense_allocations.findMany({
    select: { person_id: true, kind: true, split_mode: true, amount_cents: true, allocation_order: true },
    where: { expense_id: row.id },
    order: { allocation_order: Order.Asc }
  });
  const charges = await db.charges.findMany({
    select: CHARGE_SELECT,
    where: { source: 'expense', source_id: row.id },
  ...
  return {
    ...
    allocations: allocations.records.map((allocation) => ({
      kind: allocation.kind,
      personId: allocation.person_id ?? null,
      splitMode: allocation.split_mode,
      amount: { amountCents: allocation.amount_cents, currency: 'BRL' },
...
export async function createExpense(db: DbClient, ownerId: string, idempotencyKey: string, input: ExpenseInput): Promise<ExpenseDetail> {
  ...
  return db.transaction(async (tx) => {
    ...
    const existing = await tx.expenses.findOne({ select: EXPENSE_SELECT, where: { owner_id: ownerId, idempotency_key: idempotencyKey } });
    if (existing) {
    ...
    const row = await tx.expenses.insertOne({
      select: EXPENSE_SELECT,
      data: {
        id: expenseId,
        owner: { id: ownerId },
        type: normalized.installmentCount === 1 ? 'one_time' : 'installment',
        description: plan.description,
        total_cents: plan.totalCents,
        currency: 'BRL',
        installment_count: normalized.installmentCount,
        first_due_date: normalized.firstDueDate,
        ...(normalized.paymentMethodId ? { payment_method: { id: normalized.paymentMethodId } } : {}),
    ...
    for (const [index, allocation] of plan.allocations.entries()) {
      ...
      await tx.expense_allocations.insertOne({
        select: { id: true },
        data: {
          id: crypto.randomUUID(),
          expense: { id: expenseId },
          ...(allocation.kind === 'person' ? { person: { id: allocation.personId } } : {}),
          kind: allocation.kind,
          split_mode: normalized.split.mode,
          ...(normalized.split.mode === 'percentage' && original && 'basisPoints' in original
            ? { basis_points: original.basisPoints }
            : {}),
