...
export namespace InviteRepository {
  /**
  ...
  export async function accept(
    ...
    const result = await db.transaction(async (tx) => {
      ...
      if (!alreadyIn && (await splitHasPlaceholders(tx, split))) {
        ...
        if (parked) {
          await tx.billing_guests.updateOne({
            where: { id: parked.id },
            data: { state: BillingGuestState.Pending, created_at: instant, resolved_at: sqlNull }
          });
        } else {
      ...
      await ContactRepository.ensure(tx, billing.owner_id, userId, instant);
