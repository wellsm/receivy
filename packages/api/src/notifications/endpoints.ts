...
import type { NotificationDelivery, NotificationDevice, NotificationPreferences } from '@receivy/common';
import type { SessionIdentity } from '../authorizers/session';
import type { ApiProvider } from '../provider';
import { getPreferences, listDeliveries, listDevices, manualReminder, registerDevice, removeDevice, savePreferences } from './repository';
...
export async function getPreferencesHandler(request: Request, context: Service.Context<ApiProvider>): Promise<PreferencesResponse> {
  return {
    status: 200,
...
export async function savePreferencesHandler(
  ...
  return {
    status: 200,
    body: await savePreferences(context.db, request.identity.userId, request.body)
  };
}
export async function listDevicesHandler(request: Request, context: Service.Context<ApiProvider>): Promise<DevicesResponse> {
  return {
    status: 200,
...
export async function removeDeviceHandler(request: IdRequest, context: Service.Context<ApiProvider>): Promise<EmptyResponse> {
  await removeDevice(context.db, request.identity.userId, request.parameters.id);
  return { status: 204 };
}
