  ...
  for (const event of events.records)
    await db.transaction(async (tx) => {
      ...
      await planDelivery(
        ...
        charge,
        row.id,
        row.type === "charge.created" ? "initial" : "reminder",
        config,
        now,
      );
...
async function scheduleReminders(
  ...
  charge: ChargeRow,
  now: number,
  snapshot?: { offsets: number[]; timezone: string },
) {
  ...
  const offsets = occurrence
    ? (
        ...
        .map((r) => r.offsetDays)
    : (snapshot?.offsets ??
      (await getPreferences(db, charge.creditor_id)).reminderOffsets);
...
async function planDelivery(
  ...
  for (const recipient of recipients) {
    ...
    const data = {
        ...
        body_hash: valid
          ? digest(
              JSON.stringify(renderNotice(inputs, template, config.secret)),
            )
          : "",
        idempotency_key: key,
        attempts: 0,
