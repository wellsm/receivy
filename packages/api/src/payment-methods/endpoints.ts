...
declare class ListRequest implements Http.Request { identity: SessionIdentity; query: { archived?: boolean } }
declare class InputBody implements Http.JsonBody { pixKeyType: "cpf" | "cnpj" | "email" | "phone" | "random"; pixKey: String.Max<254>; label?: String.Max<120> }
declare class CreateRequest implements Http.Request { identity: SessionIdentity; body: InputBody }
declare class EditRequest implements Http.Request { identity: SessionIdentity; parameters: { id: String.UUID }; body: InputBody }
declare class IdRequest implements Http.Request { identity: SessionIdentity; parameters: { id: String.UUID } }
declare class ListResponse implements Http.Response { status: 200; body: PaymentMethodsPage }
declare class CreateResponse implements Http.Response { status: 201; body: PaymentMethod }
declare class ItemResponse implements Http.Response { status: 200; body: PaymentMethod }
...
export async function editPaymentMethodHandler(request: EditRequest, context: Service.Context<ApiProvider>): Promise<ItemResponse> {
  return { status: 200, body: await safe(() => savePaymentMethod(context.db, request.identity.userId, input(request.body), request.parameters.id)) };
}
