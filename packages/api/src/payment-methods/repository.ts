...
type Row = { id: string; pix_key_type: PaymentMethod["pixKeyType"]; pix_key: string; label: string; is_default: boolean; archived_at?: string; created_at: string };

function dto(row: Row): PaymentMethod {
  return { id: row.id, type: "pix", pixKeyType: row.pix_key_type, pixKey: row.pix_key, label: row.label,
    isDefault: row.is_default, archivedAt: row.archived_at ?? null, createdAt: row.created_at };
}
...
export async function listPaymentMethods(db: DbClient, ownerId: string, archived = false): Promise<PaymentMethod[]> {
  const { records } = await db.payment_methods.findMany({ select: SELECT,
    where: { owner_id: ownerId, archived_at: { isNull: !archived } }, order: { created_at: Order.Asc } });
...
export async function savePaymentMethod(db: DbClient, ownerId: string, input: PaymentMethodInput, id?: string): Promise<PaymentMethod> {
  ...
  return db.transaction(async tx => {
    ...
    const duplicate = await tx.payment_methods.findOne({ select: { id: true },
    ...
    if (existing) {
      const changed = await tx.payment_methods.updateOne({ select: { id: true }, where: { id: existing.id, owner_id: ownerId },
    ...
    const anyActive = await tx.payment_methods.findMany({ select: { id: true }, where: { owner_id: ownerId, archived_at: { isNull: true } }, take: 1 });
    const row = await tx.payment_methods.insertOne({ select: SELECT, data: { id: crypto.randomUUID(), owner: { id: ownerId },
      type: "pix", pix_key_type: input.pixKeyType, pix_key: value.key, label: value.label, is_default: !anyActive.records.length,
...
export async function makeDefaultPaymentMethod(db: DbClient, ownerId: string, id: string): Promise<PaymentMethod> {
  return db.transaction(async tx => {
    ...
    await tx.payment_methods.updateMany({ select: { id: true }, where: { owner_id: ownerId, is_default: true }, data: { is_default: false } });
    const changed = await tx.payment_methods.updateOne({ select: { id: true }, where: { id, owner_id: ownerId },
...
export async function archivePaymentMethod(db: DbClient, ownerId: string, id: string): Promise<void> {
  await db.transaction(async tx => {
    ...
    const now = new Date().toISOString();
    await tx.payment_methods.updateOne({ select: { id: true }, where: { id, owner_id: ownerId },
      data: { archived_at: now, is_default: false, updated_at: now } });
    if (target.is_default) {
      const replacement = await tx.payment_methods.findMany({ select: { id: true },
        where: { owner_id: ownerId, archived_at: { isNull: true } }, take: 1 });
      if (replacement.records[0]) await tx.payment_methods.updateOne({ select: { id: true }, where: { id: replacement.records[0].id }, data: { is_default: true, updated_at: now } });
    }
  });
}
