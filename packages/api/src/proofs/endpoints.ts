...
import { createUploadIntent, downloadProof, finalizeProof, listProofs, publicProofStatus, reviewProof } from './repository';
import { throttleProof } from './throttle';

declare class UploadRequest implements Http.Request {
  identity: SessionIdentity;
  parameters: { id: String.UUID };
  body: { filename: String.Size<1, 200>; mime: 'image/jpeg' | 'image/png' | 'application/pdf'; size: number };
}
declare class FinalizeRequest implements Http.Request {
  identity: SessionIdentity;
  parameters: { id: String.UUID; intentId: String.UUID };
}
declare class ListRequest implements Http.Request {
  identity: SessionIdentity;
  parameters: { id: String.UUID };
}
declare class ProofRequest implements Http.Request {
  identity: SessionIdentity;
  parameters: { id: String.UUID; proofId: String.UUID };
}
...
declare class PublicFinalizeRequest implements Http.Request {
  parameters: { token: String.Max<200>; intentId: String.UUID };
}
declare class UploadResponse implements Http.Response {
  status: 200;
  body: ProofUploadIntent;
}
declare class ProofResponse implements Http.Response {
  status: 200;
  body: ProofDetail;
}
declare class ProofsResponse implements Http.Response {
  status: 200;
  body: { proofs: ProofDetail[] };
}
declare class DownloadResponse implements Http.Response {
  status: 200;
  body: { url: string; expiresIn: number };
}
declare class PublicFinalizeResponse implements Http.Response {
  status: 200;
  body: { state: 'pending' };
}
declare class PublicStatusResponse implements Http.Response {
  status: 200;
  body: { state: 'pending' | 'accepted' | 'rejected'; reason: string | null; closureReason: 'paid' | 'cancelled' | null };
}
export async function publicProofStatusHandler(
...
export async function uploadProofHandler(request: UploadRequest, context: Service.Context<ApiProvider>): Promise<UploadResponse> {
  ...
  return {
    status: 200,
    body: await createUploadIntent(
      context.db,
      configuredProofStorage(context.variables),
...
export async function reviewProofHandler(request: ReviewRequest, context: Service.Context<ApiProvider>): Promise<ProofResponse> {
  return {
    status: 200,
    body: await reviewProof(context.db, request.parameters.id, request.identity.userId, request.parameters.proofId, request.body)
  };
}
export async function downloadProofHandler(request: ProofRequest, context: Service.Context<ApiProvider>): Promise<DownloadResponse> {
  return {
    status: 200,
    body: await downloadProof(
      context.db,
      configuredProofStorage(context.variables),
      request.parameters.id,
      request.identity.userId,
      request.parameters.proofId
    )
  };
}
export async function publicUploadProofHandler(
  ...
  return {
    ...
    body: await createUploadIntent(
      ...
      configuredProofStorage(context.variables),
      charge.id,
      { token: request.parameters.token, secret },
      request.body
    )
  };
}
export async function publicFinalizeProofHandler(
  request: PublicFinalizeRequest,
  context: Service.Context<ApiProvider>
): Promise<PublicFinalizeResponse> {
  const secret = context.variables.PUBLIC_LINK_HMAC_SECRET;
  await throttleProof(context.db, request.parameters.token, Date.now(), trustedClientIp(request));
  const charge = await resolvePublicCharge(context.db, request.parameters.token, secret);
  await finalizeProof(
