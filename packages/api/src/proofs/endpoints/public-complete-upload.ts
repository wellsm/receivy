...
export async function publicCompleteProofUploadHandler(
  request: PublicCompleteRequest,
  ...
  const row = await ProofRepository.completeUpload(db, bucketProofStorage(proofFiles), charge.id, actor);
