...
import { receiveProofObject } from '../repositories/proof';
...
export async function proofObjectEvent(event: Bucket.ObjectEvent, context: Service.Context<ProofFiles>): Promise<void> {
  if (event.eventType !== BucketEventType.Create) {
    return;
  }

  const outcome = await receiveProofObject(context.db, bucketProofStorage(context.proofFiles), event.objectKey);
