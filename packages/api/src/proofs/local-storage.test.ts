...
import { localProofStorage, startLocalProofServer } from './local-storage';
...
describe('explicit local private proof storage', () => {
  ...
  it('requires signatures, limits uploads, and returns attachment-only short downloads', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'receivy-proof-test-'));
    ...
    const server = await startLocalProofServer({ ...config, port: 0 });
    ...
    expect(
      (await fetch(upload, { method: 'PUT', headers: { 'content-type': 'application/pdf' }, body: 'too much data for intent' })).status
    ).toBe(422);
