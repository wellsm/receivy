...
import { MAX_PROOF_BYTES, type ProofMime, readBounded } from './validation.ts';

type Config = { directory: string; secret: string; baseUrl: string };
const KEY = /^(temporary|proofs)\/[a-f0-9-]{36}\/[a-f0-9-]{36}$/;
function file(directory: string, key: string) {
  if (!isAbsolute(directory) || !KEY.test(key)) throw new Error('Invalid local storage path.');
  return join(directory, key);
}
function signature(secret: string, payload: string) {
  if (secret.length < 32) throw new Error('Local storage signing secret is not configured.');
  return createHmac('sha256', secret).update(payload).digest('hex');
}
export function localProofStorage(config: Config): ReconciliableProofStorage {
  const url = new URL(config.baseUrl);
  if (!['127.0.0.1', 'localhost', '[::1]'].includes(url.hostname) || url.protocol !== 'http:' || !isAbsolute(config.directory))
    throw new Error('Local proof storage requires a loopback HTTP server and absolute directory.');
  ...
  return {
    async list(cursor) {
      ...
      for await (const entry of directory) {
        ...
        const key = relative(config.directory, path);
    ...
    uploadUrl: async (key, mime, size) => {
      if (!key.startsWith('temporary/') || size > MAX_PROOF_BYTES || size <= 0) throw new Error('Invalid upload.');
      return sign(key, 'PUT', mime, size, 300);
    },
    read: (key) => readBounded(createReadStream(file(config.directory, key))),
    async write(key, bytes) {
      const path = file(config.directory, key);
      await mkdir(dirname(path), { recursive: true });
      await writeFile(path, bytes, { flag: 'wx', mode: 0o600 });
    },
    async delete(key) {
      await unlink(file(config.directory, key)).catch((error) => {
        if (error.code !== 'ENOENT') throw error;
      });
    },
    downloadUrl: async (key, mime) => {
      if (!key.startsWith('proofs/')) throw new Error('Only finalized proofs can be downloaded.');
      return sign(key, 'GET', mime, 0, 60);
    }
  };
}
...
export async function startLocalProofServer(config: { directory: string; secret: string; origin: string; port: number }) {
  ...
  const server = createServer(async (request, response) => {
    ...
    if (request.method === 'OPTIONS') {
      response.writeHead(request.headers.origin === config.origin ? 204 : 403).end();
      return;
    }
    try {
      const url = new URL(request.url ?? '', 'http://localhost');
      const parts = (url.searchParams.get('grant') ?? '').split('.');
      const [payload, supplied] = parts;
      const expected = payload ? signature(config.secret, payload) : '';
      if (
      ...
      if (grant.method !== request.method || grant.expires <= Date.now()) {
        response.writeHead(403).end();
        return;
      }
      const path = file(config.directory, grant.key);
      if (request.method === 'PUT' && grant.key.startsWith('temporary/')) {
        if (request.headers['content-type'] !== grant.mime || Number(request.headers['content-length']) !== grant.size) {
        ...
        response.setHeader(
          'Content-Disposition',
          `attachment; filename="comprovante.${grant.mime === 'application/pdf' ? 'pdf' : grant.mime === 'image/png' ? 'png' : 'jpg'}"`
        );
