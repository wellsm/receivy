...
import { type ProofMime, readBounded } from './validation';
    ...
    async list(cursor) {
      ...
      return {
        objects: (response.Contents ?? []).flatMap((object) =>
          object.Key && object.LastModified ? [{ key: object.Key, modifiedAt: object.LastModified.toISOString() }] : []
        ),
        cursor: response.IsTruncated ? (response.NextContinuationToken ?? null) : null
      };
    },
    uploadUrl: (key, mime, size) =>
      getSignedUrl(client, new PutObjectCommand({ Bucket: bucket, Key: key, ContentType: mime, ContentLength: size }), {
        expiresIn: 300,
    ...
    downloadUrl: (key, mime) =>
      getSignedUrl(
        ...
        new GetObjectCommand({
          ...
          ResponseContentDisposition: `attachment; filename="comprovante.${mime === 'application/pdf' ? 'pdf' : mime === 'image/png' ? 'png' : 'jpg'}"`,
