...
describe("proof bytes", () => {
  ...
  it("cancels oversized streams before buffering the entire object", async () => {
    let closed = false;
    async function* source() { try { yield Buffer.alloc(8); yield Buffer.alloc(8); throw new Error("must not read more"); } finally { closed = true; } }
    await expect(readBounded(source(), 10)).rejects.toThrow("10 MB");
    expect(closed).toBe(true);
    expect(await readBounded((async function* () { yield Buffer.from("abc"); })())).toEqual(Buffer.from("abc"));
  });
});
