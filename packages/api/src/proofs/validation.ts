...
export function validateProof(bytes: Buffer, claimedMime: string) {
  ...
  const mime: ProofMime | undefined = bytes.subarray(0, 3).equals(Buffer.from([0xff, 0xd8, 0xff])) ? "image/jpeg"
    : bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])) ? "image/png"
      : bytes.subarray(0, 5).toString() === "%PDF-" ? "application/pdf" : undefined;
