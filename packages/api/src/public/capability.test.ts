...
describe("public charge capability", () => {
  ...
  it("requires an explicitly configured secret", () => {
    expect(() => issuePublicChargeToken({ publicId, version: 1, expiresAtSeconds: 2_000, secret: "" })).toThrow("Public link secret is not configured");
    expect(() => issuePublicChargeToken({ publicId, version: 1, expiresAtSeconds: 2_000, secret: "disabled" })).toThrow("Public link secret is not configured");
  });
});
