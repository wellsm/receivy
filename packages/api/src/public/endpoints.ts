...
declare class PublishRequest implements Http.Request { identity: SessionIdentity; parameters: { id: String.UUID }; body?: { paymentMethodId?: String.UUID } }
...
export async function createPublicLinkHandler(request: PublishRequest, context: Service.Context<ApiProvider>): Promise<LinkResponse> {
  return { status: 200, body: await createOrRotatePublicLink(context.db, request.identity.userId, request.parameters.id,
    context.variables.PUBLIC_LINK_HMAC_SECRET, false, undefined, request.body?.paymentMethodId) };
}
