...
declare class RecurrenceBody implements Http.JsonBody {
  description?: String.Max<500>; totalCents: Integer.Min<1>; frequency: "monthly" | "yearly";
  day: Integer.Range<1, 31>; month?: Integer.Range<1, 12>; startDate?: String.Date; endDate?: String.Date;
  timezone: String.Max<100>; paymentMethodId?: String.UUID;
  split: { mode: "fixed" | "equal" | "percentage"; parts: ({ kind: "owner"; basisPoints?: number } | { kind: "person"; personId: String.UUID; amountCents?: number; basisPoints?: number })[] };
  reminders?: { offsetDays: Integer.Range<-90, 90>; channel: "auto"; enabled: boolean }[];
}
declare class CreateRequest implements Http.Request { identity: SessionIdentity; headers: { "idempotency-key": String.Max<200> }; body: RecurrenceBody }
declare class ReadRequest implements Http.Request { identity: SessionIdentity; parameters: { id: String.UUID } }
declare class EditRequest implements Http.Request { identity: SessionIdentity; parameters: { id: String.UUID }; body: RecurrenceBody }
declare class ListRequest implements Http.Request { identity: SessionIdentity }
declare class DetailResponse implements Http.Response { status: 200; body: RecurrenceDetail }
declare class CreateResponse implements Http.Response { status: 201; body: RecurrenceDetail }
declare class ListResponse implements Http.Response { status: 200; body: RecurrencesPage }
async function validation<T>(operation: () => Promise<T>): Promise<T> { try { return await operation(); } catch (error) { if (error instanceof RangeError) throw new HttpBadRequestError(error.message); throw error; } }
export async function createRecurrenceHandler(request: CreateRequest, context: Service.Context<ApiProvider>): Promise<CreateResponse> {
  return { status: 201, body: await validation(() => createRecurrence(context.db, request.identity.userId, request.headers["idempotency-key"], request.body as RecurrenceInput)) };
}
export async function editRecurrenceHandler(request: EditRequest, context: Service.Context<ApiProvider>): Promise<DetailResponse> {
  return { status: 200, body: await validation(() => editRecurrence(context.db, request.identity.userId, request.parameters.id, request.body as RecurrenceInput)) };
}
