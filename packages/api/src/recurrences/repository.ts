...
import { persistChargePlan, prepareChargeMaterialization } from '../expenses/repository';
...
async function lockOwner(db: DbClient, ownerId: string) {
  ...
  if (!(await db.users.findOne({ select: { id: true }, where: { id: ownerId, deleted_at: { isNull: true } }, lock: true })))
...
async function ruleInput(
  db: DbClient,
  row: RuleRow
): Promise<RecurrenceInput & { startDate: string; description: string; reminders: NonNullable<RecurrenceInput['reminders']> }> {
  const allocations = (
    await db.recurrence_allocations.findMany({
      select: { kind: true, person_id: true, split_mode: true, amount_cents: true, basis_points: true },
      where: { recurrence_id: row.id },
      order: { allocation_order: Order.Asc }
    })
  ).records;
  const mode = allocations[0]?.split_mode ?? 'equal';
  const parties = allocations.map((a) =>
    a.kind === 'owner' ? { kind: 'owner' as const } : { kind: 'person' as const, personId: a.person_id! }
  );
  const split: ExpenseSplit =
    mode === 'fixed'
      ? {
          mode,
          parts: allocations.flatMap((a) =>
            a.kind === 'person' ? [{ kind: 'person' as const, personId: a.person_id!, amountCents: a.amount_cents }] : []
          )
        }
      : mode === 'equal'
        ? { mode, parts: parties }
        : { mode, parts: parties.map((p, i) => ({ ...p, basisPoints: allocations[i]!.basis_points! })) };
  const reminders = (
    await db.recurrence_reminders.findMany({
      select: { offset_days: true, channel: true, enabled: true },
      where: { recurrence_id: row.id },
      order: { offset_days: Order.Asc }
    })
  ).records.map((r) => ({ offsetDays: r.offset_days, channel: r.channel, enabled: r.enabled }));
  return {
...
async function previews(db: DbClient, row: RuleRow, input: Awaited<ReturnType<typeof ruleInput>>, now: Date): Promise<RecurrencePreview[]> {
  ...
  const existing = (
    await db.recurrence_occurrences.findMany({
      select: { occurrence_date: true },
      where: { recurrence_id: row.id, occurrence_date: { gte: today } }
    })
  ).records.map((r) => r.occurrence_date);
  ...
  return dates
    .filter((d) => !existing.includes(d) && d > row.processed_through)
    .map((occurrenceDate) => ({
      recurrenceId: row.id,
      description: row.description,
      amount: { amountCents: external, currency: 'BRL' },
      occurrenceDate,
...
async function dto(db: DbClient, row: RuleRow, now: Date): Promise<RecurrenceDetail> {
  ...
  const existing =
    row.state === 'active'
      ? (
          await db.recurrence_occurrences.findMany({
            select: { occurrence_date: true },
            where: { recurrence_id: row.id, occurrence_date: { gt: row.processed_through } }
          })
        ).records.map((r) => r.occurrence_date)
      : [];
  const next =
    row.state === 'active'
      ? recurrenceDates(input, addCalendarDays(row.processed_through, 1), '9999-12-31', existing.length + 1).find(
  ...
  return {
    ...
    nextMaterialization: next ? materializationDate(next, input.reminders) : null,
    previews: projected
  };
}
async function audit(db: DbClient, ownerId: string, id: string, type: string, now: string) {
  await db.activity_events.insertOne({
    data: {
      ...
      aggregate_type: 'recurrence',
      aggregate_id: id,
      type,
      payload: JSON.stringify({ recurrenceId: id }),
      created_at: now
    }
  });
}
async function saveParts(db: DbClient, id: string, input: ReturnType<typeof normalizeRecurrenceInput> & { startDate: string }) {
  ...
  for (const [index, part] of plan.allocations.entries()) {
    ...
    await db.recurrence_allocations.insertOne({
      data: {
        id: crypto.randomUUID(),
        recurrence: { id },
        kind: part.kind,
  ...
  for (const reminder of input.reminders)
    await db.recurrence_reminders.insertOne({
      data: {
        id: crypto.randomUUID(),
        recurrence: { id },
...
async function validateParties(db: DbClient, ownerId: string, input: RecurrenceInput) {
  await prepareChargeMaterialization(
    db,
    ownerId,
    input.split.parts.flatMap((p) => (p.kind === 'person' ? [p.personId] : [])),
...
export async function createRecurrence(
  ...
  return db.transaction(async (tx) => {
    ...
    if (existing) {
      if (existing.request_hash !== hash) throw new HttpConflictError('Idempotency-Key já usada com outro conteúdo.');
    ...
    const row = await tx.recurrences.insertOne({
      select: SELECT,
      data: {
        ...fields(input),
        id,
        ...
        currency: 'BRL',
        state: 'active',
        processed_through: addCalendarDays(today, -1),
        idempotency_key: key,
        request_hash: hash,
        created_at: instant,
        updated_at: instant
    ...
    await saveParts(tx, id, input);
    await audit(tx, ownerId, id, 'recurrence.created', instant);
    return dto(tx, row, now);
  });
}
export async function getRecurrence(db: DbClient, ownerId: string, id: string, now = new Date()) {
  return dto(db, await ruleRow(db, ownerId, id), now);
}
...
export async function editRecurrence(db: DbClient, ownerId: string, id: string, raw: RecurrenceInput, now = new Date()) {
  ...
  return db.transaction(async (tx) => {
    await lockOwner(tx, ownerId);
    const row = await ruleRow(tx, ownerId, id, true);
    ...
    const updated = await tx.recurrences.updateOne({
      select: SELECT,
      where: { id },
      data: {
        ...fields(input),
        processed_through: row.processed_through < boundary ? row.processed_through : boundary,
        updated_at: now.toISOString()
      }
    });
    await saveParts(tx, id, input);
    await audit(tx, ownerId, id, 'recurrence.edited', now.toISOString());
...
export async function transitionRecurrence(db: DbClient, ownerId: string, id: string, state: RecurrenceDetail['state'], now = new Date()) {
  return db.transaction(async (tx) => {
    ...
    if (!updated) throw new HttpNotFoundError();
    await audit(tx, ownerId, id, `recurrence.${state}`, now.toISOString());
    return dto(tx, await ruleRow(tx, ownerId, id), now);
  });
}
export async function materializeRecurrences(db: DbClient, now = new Date()): Promise<{ materialized: number; failures: string[] }> {
  const candidates = (
    await db.recurrences.findMany({
      select: { id: true, owner_id: true, last_attempted_at: true },
      where: { state: 'active' },
      order: { id: Order.Asc }
    })
  ).records;
  ...
  while (queue.length && evaluated < 100) {
    ...
    try {
      const result = await db.transaction(async (tx) => {
        ...
        const input = await ruleInput(tx, row);
        const today = calendarDate(now, row.timezone);
        ...
        for (const dueDate of dates) {
          ...
          attempted = true;
          evaluated++;
          const existing = await tx.recurrence_occurrences.findOne({
            select: { id: true },
            where: { recurrence_id: row.id, occurrence_date: dueDate }
          });
          if (!existing) {
            const context = await prepareChargeMaterialization(
              tx,
              row.owner_id,
              input.split.parts.flatMap((p) => (p.kind === 'person' ? [p.personId] : [])),
              input.paymentMethodId
            ...
            const id = crypto.randomUUID();
            const instant = now.toISOString();
            await tx.recurrence_occurrences.insertOne({
              data: {
                id,
                recurrence: { id: row.id },
                occurrence_date: dueDate,
                materialized_at: instant,
                reminders_json: JSON.stringify(input.reminders),
                timezone: row.timezone
              }
            });
            ...
            await tx.activity_events.insertOne({
              data: {
                id: crypto.randomUUID(),
                actor_user: { id: row.owner_id },
                subject_user: { id: row.owner_id },
                type: 'recurrence.materialized',
                aggregate_type: 'recurrence_occurrence',
                aggregate_id: id,
                payload: JSON.stringify({ recurrenceId: row.id, occurrenceId: id, dueDate }),
                created_at: instant
              }
            });
