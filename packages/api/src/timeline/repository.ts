...
function accessWhere(userId: string, email?: string) {
  return { OR: [{ creditor_id: userId }, { recipient_user_id: userId }, ...(email ? [{ recipient_email_snapshot: email }] : [])] };
}
  ...
  const access =
    filters.direction === 'receivable'
      ...
      : filters.direction === 'payable'
        ? { OR: [{ recipient_user_id: userId }, ...(email ? [{ recipient_email_snapshot: email }] : [])] }
...
function directionFor(row: ChargeRow, userId: string): Direction {
  return row.creditor_id === userId ? 'receivable' : 'payable';
}
...
export async function getTimeline(db: DbClient, userId: string, filters: TimelineFilters): Promise<TimelinePage> {
  ...
  const allQuery = await db.charges.findMany({
    ...
    where: visibleWhere(userId, user.verified_email, filters, false, today)
  });
  ...
  const proofsToReview = receivableIds.length
    ? await db.payment_proofs.count({ where: { charge_id: { isIn: receivableIds }, state: 'pending' } })
    : 0;
  return {
    items: [
      ...primaryPage.map(
...
export async function getPersonLedger(db: DbClient, userId: string, personId: string, cursor?: string): Promise<PersonLedger> {
  ...
  const baseWhere = {
    OR: [
      ...
      ...(person.linked_user_id
        ? [
            {
              creditor_id: person.linked_user_id,
              OR: [{ recipient_user_id: userId }, ...(current.verified_email ? [{ recipient_email_snapshot: current.verified_email }] : [])]
            }
          ]
        : [])
    ]
  ...
  return {
    personId,
    person: await getPerson(db, userId, personId),
