...
export async function getRemindersHandler({ identity }: RemindersRequest, { accounts }: Service.Context<UserProvider>): Promise<RemindersResponse> {
  return { status: 200, body: await accounts.reminders(identity.userId) };
}
