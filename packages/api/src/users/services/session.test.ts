...
import { generateRefreshToken, hashRefreshToken, issueAccessToken, verifyAccessToken } from './session';
...
describe('session token security', () => {
  ...
  it('rejects a token whose signed payload was changed', () => {
    ...
    const tamperedPayload = Buffer.from(JSON.stringify({ sub: 'attacker', exp: 9_999_999_999 })).toString('base64url');
