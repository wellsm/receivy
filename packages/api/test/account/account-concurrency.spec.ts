...
describe("account erasure versus cross-account materialization", () => {
  ...
  for (const first of ["materialization", "erasure"] as const) {
    it(`${first} first: no FK/person deadlock and no charge missed by erasure`, async () => {
      const creditor = crypto.randomUUID(); const recipient = crypto.randomUUID();
      ...
      const entered = deferred<{ backend: number; tx: DbClient }>(); const release = deferred<void>(); const other = deferred<number>();
      let paused = false;
            ...
            if (property === (first === "materialization" ? "people" : "users")) {
              ...
              return new Proxy(table, {
                get(relation, operation, relationReceiver) {
                  if (operation === "findOne") return async (...args: unknown[]) => {
      ...
      const secondClient = observeTransactions(db, async tx => { other.resolve(await pid(tx)); return tx; });
      const materialize = (client: DbClient) => createExpense(client, creditor, `concurrent-${recipient}`, {
        totalCents: 1234, installmentCount: 1, firstDueDate: "2026-10-01",
      ...
      try { await waitBlocked(firstStarted.tx, await other.promise, firstStarted.backend); } finally { release.resolve(); }
      const outcomes = await completed;
      if (first === "materialization") {
        deepEqual(outcomes.map(result => result.status), ["fulfilled", "fulfilled"],
          outcomes.filter(result => result.status === "rejected").map(result => String(result.reason)).join("; "));
      } else {
        equal(outcomes[0].status, "fulfilled");
        equal(outcomes[1].status, "rejected", "archived recipient prevents a new post-erasure charge");
      }
      const charges = await db.charges.findMany({ select: { amount_cents: true, state: true, recipient_user_id: true, recipient_email_snapshot: true }, where: { creditor_id: creditor } });
      equal(charges.records.length, first === "materialization" ? 1 : 0);
      for (const charge of charges.records) {
        equal(charge.amount_cents, 1234); equal(charge.state, "pending");
        equal(charge.recipient_user_id, null); equal(charge.recipient_email_snapshot, null);
      }
    });
  }
});
