...
import { registerDevice } from '../../src/notifications/services/notification';
import { ProofRepository } from '../../src/proofs/repositories/proof';
import { actorHash } from '../../src/proofs/utils/slot';
...
async function attachProof(
  ...
  await db.proofs.insertOne({
    data: {
      ...
      actor_hash: sender
        ? actorHash({ userId: sender })
        : actorHash({ token: 'public-fixture', secret }),
      sent_at: now,
      created_at: now,
      updated_at: now
    }
...
async function proofColumns(chargeId: string) {
  const row = await ProofRepository.current(db, chargeId);
...
describe('account lifecycle on dedicated PostgreSQL', () => {
  ...
  it('rejects issued access after remote revocation and forbids foreign or missing families', async () => {
    ...
    await rejects(() => revokeFamily(db, debtor, current.familyId), HttpForbiddenError);
    await revokeFamily(db, owner, current.familyId);
  ...
  it('erases the account and reports even a legacy proof key the bucket will not know', async () => {
    ...
    const person = await contacts.save(owner, { name: 'Rollback fixture', email: 'rollback-account@example.com' });
    ...
    deepEqual(objectKeys, [avatarKey(id), avatarStagingKey(id), 'invalid-legacy-key']);

    await rejects(() => authorize(auth.access), HttpUnauthorizedError);

    equal((await charges.get(owner, rollbackChargeId)).recipient.email, null);
  ...
  it("atomically erases identity, preserves other account's payment fact and closes re-registration history access", async () => {
    ...
    equal((await charges.get(debtor, chargeId)).direction, 'payable');
    ...
    const row = await charges.get(owner, chargeId);
    ...
    equal((await publicChargeByToken(db, publicLink.token, secret)).state, 'paid');
    ...
    deepEqual(
      ...
      [avatarKey(debtor), avatarStagingKey(debtor), ownKey]
    );
    ...
    await rejects(() => charges.get(replacement.id, chargeId), HttpForbiddenError);
    await rejects(
      () => contacts.save(debtor, { name: 'Stale request', email: 'stale-contact@example.com' }),
      HttpUnauthorizedError,
