...
import { personOf } from '../../src/contacts/utils/person';
import { AccountRepository } from '../../src/users/repositories/account';
import { charges, cleanupUsers, contacts, createOnceCharge, createUser, db } from '../fixtures/financial';
...
describe('avatars on people', () => {
  ...
  before(async () => {
    ...
    await contacts.save(owner, { name: 'Devedor', email: 'debtor-avatar@example.test' });
  ...
  it('references the debtor photo on the charge detail', async () => {
  ...
  it('references no photo for a person without one', async () => {
    const detail = await charges.get(debtor, chargeId);

    equal(detail.counterpartAvatar, null);
  });

  it('references the photo on the owner contact', async () => {
    const person = personOf(await AccountRepository.person(db, debtor));
