...
describe("atomic authentication on PostgreSQL", () => {
  ...
  it("rolls back code consumption, new user and linkage if session persistence fails", async () => {
    ...
    const result = outcomes.find(x => x.status === "fulfilled"); ok(result?.status === "fulfilled");
  ...
  it("commits failed OTP attempt counters despite returning a generic failure", async () => {
    await db.login_codes.updateOne({ where: { email }, data: { consumed_at: null as unknown as undefined, attempts: 0, expires_at: new Date(Date.now() + 600000).toISOString() } });
    for (let index = 0; index < 5; index++) await rejects(() => confirmEmailAtomically(db, { email, code: "000000" }, config), AuthFlowError);
  ...
  it("rolls back grant consumption on session failure and serializes concurrent successful exchange", async () => {
    const code = randomUUID(); const codeVerifier = "v".repeat(43); const userId = (await repo.findOrCreateUserByEmail(email)).id;
    ids.push(userId);
    await repo.createGrant({ userId, grantHash: hashOauthValue(code), clientChallenge: hashOauthValue(codeVerifier), expiresAt: new Date(Date.now() + 120000) });
    await rejects(() => exchangeOauthAtomically(db, { code, codeVerifier, deviceName: "x".repeat(121) }, config));
    equal((await db.oauth_grants.findOne({ select: { consumed_at: true }, where: { grant_hash: hashOauthValue(code) } }))?.consumed_at, null);
