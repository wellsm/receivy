...
describe("trusted source IP quotas on PostgreSQL", () => {
  it("counts invalid capabilities before lookup so guesses cannot bypass the quota", async () => {
    const context = { db, variables: { PUBLIC_LINK_HMAC_SECRET: "throttle-test-secret-only" } } as Parameters<typeof publicUploadProofHandler>[1];
    const request = { sourceIp: "192.0.2.211", parameters: { token: `invalid-${crypto.randomUUID()}` }, body: { filename: "fixture.pdf", mime: "application/pdf" as const, size: 12 } };
    for (let index = 0; index < 12; index++) await rejects(() => publicUploadProofHandler(request, context), error => (error as { status: number }).status === 404);
    await rejects(() => publicUploadProofHandler(request, context), error => (error as { status: number }).status === 429);
  });
  it("isolates real IP buckets while a capability quota survives IP changes", async () => {
    ...
    equal(rows.records.every(row => /^[a-f0-9]{64}$/.test(row.id)), true);
