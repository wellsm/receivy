...
describe('financial repositories on PostgreSQL', () => {
  ...
  it('binds idempotency to payload and persists immutable recipient/Pix snapshots', async () => {
    ...
    await rejects(
      () =>
        createExpense(db, DEBTOR, 'foreign-pix', {
          ...input,
    ...
    await rejects(
      () =>
        createExpense(db, OWNER, 'archived-pix', {
          ...input,
  ...
  it('serializes integral payment and preserves terminal charge state with audit', async () => {
    ...
    const expense = await createExpense(db, OWNER, 'payment-concurrency', {
      totalCents: 1_000,
      installmentCount: 1,
    ...
    const cancellable = await createExpense(db, OWNER, 'charge-cancellation', {
      totalCents: 750,
      installmentCount: 1,
  ...
  it('returns one persisted result for simultaneous identical expense idempotency keys', async () => {
    ...
    const [first, second] = await Promise.all([
      createExpense(db, OWNER, 'same-key-race', input),
      createExpense(db, OWNER, 'same-key-race', input)
    ]);
  ...
  it('expires, rotates and revokes versioned public capabilities without leaking private fields', async () => {
    ...
    const expense = await createExpense(db, OWNER, 'public-capability', {
      totalCents: 2_500,
      installmentCount: 1,
  ...
  it('returns account-relative filtered timeline totals and a person ledger', async () => {
    ...
    const expense = await createExpense(db, OWNER, 'timeline-ledger', {
      totalCents: 6_001,
      installmentCount: 2,
