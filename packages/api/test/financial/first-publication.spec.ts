...
describe('explicit first Pix publication', () => {
  ...
  it('keeps no-Pix creation possible but refuses first sharing and makes initial notice visibly wait', async () => {
    equal((await ChargeRepository.get(db, owner, chargeId)).pix, null);
    equal((await ChargeRepository.get(db, owner, chargeId)).sharingState, 'pix_required');
    await rejects(() => PublicLinkRepository.createOrRotate(db, owner, chargeId, secret), ApiError);
  ...
  it('assigns only an owned selected Pix snapshot once, releases notice once and preserves published history', async () => {
    const foreign = await PaymentMethodRepository.save(db, other, { pixKeyType: PixKeyType.Email, pixKey: 'foreign@example.com' });
    ...
    deepEqual(a, b);
    equal((await ChargeRepository.get(db, owner, chargeId)).pix?.key, 'first@example.com');
    equal((await ChargeRepository.get(db, owner, chargeId)).sharingState, 'ready');
    await rejects(() => PublicLinkRepository.createOrRotate(db, owner, chargeId, secret, false, undefined, second.id), ApiError);
    ...
    await PublicLinkRepository.createOrRotate(db, owner, chargeId, secret, false, undefined, first.id, context);
    equal(sent.emails.length, 1, 'a later publication never repeats the hello');
    await PaymentMethodRepository.save(db, owner, { pixKeyType: PixKeyType.Email, pixKey: 'edited@example.com' }, first.id);
    equal((await ChargeRepository.get(db, owner, chargeId)).pix?.key, 'first@example.com');
  ...
  it('never fills a previously published null-Pix record, including a revoked tombstone', async () => {
    ...
    equal((await ChargeRepository.get(db, owner, chargeId)).sharingState, 'legacy_without_pix');
    const method = await PaymentMethodRepository.save(db, owner, { pixKeyType: PixKeyType.Email, pixKey: 'legacy@example.com' });
    await rejects(() => PublicLinkRepository.createOrRotate(db, owner, chargeId, secret, true, undefined, method.id), ApiError);
    equal((await ChargeRepository.get(db, owner, chargeId)).pix, null);
  });
});
