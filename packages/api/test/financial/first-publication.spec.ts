...
describe('explicit first Pix publication', () => {
  ...
  it('keeps no-Pix creation possible but refuses first sharing and makes initial notice visibly wait', async () => {
    equal((await charges.get(owner, chargeId)).payment, null);
  ...
  it('assigns only an owned selected Pix snapshot once, releases notice once and preserves published history', async () => {
    const foreign = await paymentMethods.save(other, { provider: PaymentProvider.Pix, kind: PixKeyType.Email, value: 'foreign@example.com' });
    const first = await paymentMethods.save(owner, { provider: PaymentProvider.Pix, kind: PixKeyType.Email, value: 'first@example.com' });
    const second = await paymentMethods.save(owner, { provider: PaymentProvider.Pix, kind: PixKeyType.Email, value: 'second@example.com' });
    ...
    const contactKey = await paymentMethods.save(owner, {
      ...
      value: 'publication-padaria@example.com',
    ...
    deepEqual(a, b);
    equal((await charges.get(owner, chargeId)).payment?.value, 'first@example.com');
    ...
    await paymentMethods.save(owner, { provider: PaymentProvider.Pix, kind: PixKeyType.Email, value: 'edited@example.com' }, first.id);
  ...
  it('never fills a previously published null-Pix record, including a revoked tombstone', async () => {
    ...
    const method = await paymentMethods.save(owner, { provider: PaymentProvider.Pix, kind: PixKeyType.Email, value: 'legacy@example.com' });
