...
import { createRecurrence, getRecurrence, listRecurrences, editRecurrence, transitionRecurrence, materializeRecurrences } from "../../src/recurrences/repository";
...
describe("recurrences on native PostgreSQL", () => {
  before(async () => {
    const [database] = await db.rawQuery("SELECT current_database() AS name"); equal(database?.["name"], "receivy_tests");
    ...
    input = { description: "Mensal", totalCents: 1001, frequency: "monthly", day: 31, timezone: "America/Sao_Paulo", startDate: "2026-01-01", paymentMethodId: pix.id,
  ...
  it("serializes job replay with expense creation and preserves all snapshots and one initial outbox", async () => {
    ...
    await Promise.all([materializeRecurrences(db, date("2026-01-28")), materializeRecurrences(db, date("2026-01-28")),
    ...
    const charges = await db.charges.findMany({ select: { id: true }, where: { source_id: rule.id } }); equal(charges.records.length, 1);
    ...
    await savePaymentMethod(db, OWNER, { pixKeyType: "random", pixKey: "11111111-1111-4111-8111-111111111111", label: "Editada" }, input.paymentMethodId);
    await materializeRecurrences(db, date("2026-01-28"));
    equal(await db.recurrence_occurrences.count({ where: { recurrence_id: rule.id } }), 2, "future edit must not skip nearer unmaterialized date behind the previous cursor");
    ...
    const snapshot = await getCharge(db, OWNER, id); equal(snapshot.description, "Mensal"); equal(snapshot.amount.amountCents, 501); equal(snapshot.dueDate, "2026-01-31");
    equal(snapshot.pix?.key, "52998224725"); equal(snapshot.recipient.name, "Original");
    ...
    const occurrence = await db.recurrence_occurrences.findOne({ select: { id: true, reminders_json: true }, where: { recurrence_id: rule.id, occurrence_date: "2026-01-31" } });
    ok(occurrence); equal(JSON.parse(occurrence.reminders_json).length, 3);
    ...
    await rejects(() => db.recurrence_occurrences.insertOne({ data: { id: crypto.randomUUID(), recurrence: { id: rule.id }, occurrence_date: "2026-01-31", materialized_at: date("2026-01-28").toISOString(), reminders_json: "[]", timezone: input.timezone } }));
    await transitionRecurrence(db, OWNER, rule.id, "ended");
  });
  it("clears optional end, month and Pix fields on future edits", async () => {
    const rule = await createRecurrence(db, OWNER, "clear-options", { ...input, frequency: "yearly", month: 2, day: 29, endDate: "2027-12-31" }, date("2026-01-01"));
    const edited = await editRecurrence(db, OWNER, rule.id, { ...input, paymentMethodId: undefined }, date("2026-01-01"));
    equal(edited.endDate, undefined); equal(edited.month, undefined); equal(edited.paymentMethodId, undefined);
    await transitionRecurrence(db, OWNER, rule.id, "ended");
  });
  it("rolls back unavailable recipient materialization without starving another active rule", async () => {
    ...
    const invalid = await createRecurrence(db, OWNER, "unavailable", { ...input, day: 15, split: { mode: "equal", parts: [{ kind: "person", personId: person.id }] } }, date("2026-01-01"));
    ...
    deepEqual(result.failures, [invalid.id]); equal(result.materialized, 1);
  ...
  it("skips paused historical debt but catches up continuously active outages in bounded batches", async () => {
    ...
    const occurrences = await db.recurrence_occurrences.findMany({ select: { occurrence_date: true }, where: { recurrence_id: paused.id } });
    deepEqual(occurrences.records.map(r => r.occurrence_date), ["2026-04-30"]);
  ...
  it("projects owner-only virtual entries without amounts in balances or duplication after materialization", async () => {
    const now = new Date(); const local = new Intl.DateTimeFormat("en-CA", { timeZone: input.timezone }).format(now);
    const rule = await createRecurrence(db, OWNER, "preview", { ...input, startDate: local, day: Number(local.slice(8)), reminders: [] }, now);
    ok(rule.previews.length > 0); ok(rule.previews.length <= 4);
    ...
    equal(after.items.some(i => i.kind === "recurrence_preview" && i.preview.recurrenceId === rule.id && i.preview.occurrenceDate === local), false);
    equal((await getTimeline(db, OTHER, {})).items.some(i => i.kind === "recurrence_preview"), false);
    await transitionRecurrence(db, OWNER, rule.id, "ended");
  });
});
