...
const input = {
  ...
  split: { mode: 'equal' as const, parts: [{ kind: 'person' as const, personId: 'ana' }, { kind: 'owner' as const }] }
};
...
describe('charge plan', () => {
  ...
  it('clamps non-leap February and rolls December into the following year', () => {
    expect(planExpenseCharges({ ...input, firstDueDate: '2026-12-31' }).charges.map((charge) => charge.dueDate)).toEqual([
      '2026-12-31',
      '2027-01-31',
      '2027-02-28'
    ]);
