import { resolveExpenseSplit, type ExpenseSplit, type ResolvedAllocation } from "./split";
...
export function planExpenseCharges(input: ExpensePlanInput): ExpensePlan {
  ...
  const charges = allocations.flatMap(allocation => {
    ...
    return allocation.installments.flatMap((amountCents, index): PlannedCharge[] => amountCents === 0 ? [] : [{
