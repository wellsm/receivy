...
import { materializationDate, normalizeRecurrenceInput, recurrenceDates } from './recurrence';
...
describe('recurrence civil calendar', () => {
  ...
  it('clamps Feb 29 yearly and restores leap day', () => {
    expect(recurrenceDates({ ...rule, frequency: 'yearly', day: 29, month: 2 }, '2027-01-01', '2028-12-31')).toEqual([
      '2027-02-28',
      '2028-02-29'
    ]);
  });
  it('uses Gregorian leap rules including years below 100', () => {
    expect(
      recurrenceDates({ ...rule, frequency: 'yearly', day: 29, month: 2, startDate: '0004-01-01' }, '0004-01-01', '0004-12-31')
    ).toEqual(['0004-02-29']);
  });
  it('honors inclusive start/end and bounds iteration without dropping a backlog', () => {
    expect(recurrenceDates({ ...rule, startDate: '2026-02-01', endDate: '2026-03-31' }, '2026-01-01', '2030-12-31', 1)).toEqual([
      '2026-02-28'
    ]);
  });
  it('uses enabled reminder offsets and due date only when none are enabled', () => {
    expect(
      materializationDate('2026-03-09', [
        { offsetDays: -3, channel: 'auto', enabled: false },
        { offsetDays: 2, channel: 'auto', enabled: true }
      ])
    ).toBe('2026-03-11');
