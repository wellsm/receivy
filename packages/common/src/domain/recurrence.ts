...
export function normalizeRecurrenceInput(input: RecurrenceInput) {
  ...
  if (
    ...
    input.day < 1 ||
    input.day > 31 ||
    !['monthly', 'yearly'].includes(input.frequency)
  )
    throw new RangeError('Frequência ou dia inválido.');
  if (input.frequency === 'yearly' && (!Number.isInteger(input.month) || input.month! < 1 || input.month! > 12))
  ...
  if (
    reminders.length > 10 ||
    reminders.some(
      (r) => !Number.isInteger(r.offsetDays) || Math.abs(r.offsetDays) > 90 || r.channel !== 'auto' || typeof r.enabled !== 'boolean'
    ) ||
    new Set(reminders.map((r) => r.offsetDays)).size !== reminders.length
  )
    throw new RangeError('Lembretes inválidos: use dias únicos entre -90 e 90.');
  return {
    ...input,
    description,
    reminders: [...reminders].sort((a, b) => a.offsetDays - b.offsetDays),
    ...(input.frequency === 'monthly' ? { month: undefined } : {})
  };
}
