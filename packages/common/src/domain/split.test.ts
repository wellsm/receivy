...
describe("expense split", () => {
  it("allocates the total before installments, keeping stable cent remainders", () => {
    expect(resolveExpenseSplit(100, 3, { mode: "equal", parts: [ana, bia, owner] }))
      .toEqual([
  ...
  it("assigns the fixed remainder to the owner", () => {
    expect(resolveExpenseSplit(100, 2, { mode: "fixed", parts: [
      { ...ana, amountCents: 41 }, { ...bia, amountCents: 20 },
    ] })).toEqual([
  ...
  it("awards percentage residuals by largest remainder, not input order", () => {
    expect(resolveExpenseSplit(101, 1, { mode: "percentage", parts: [
      { ...ana, basisPoints: 2000 }, { ...bia, basisPoints: 3000 },
      { ...owner, basisPoints: 5000 },
    ] }).map(part => part.amountCents)).toEqual([20, 30, 51]);
  });

  it("breaks percentage ties by stable allocation order", () => {
    expect(resolveExpenseSplit(1, 1, { mode: "percentage", parts: [
      { ...ana, basisPoints: 5000 }, { ...bia, basisPoints: 5000 },
    ] }).map(part => part.amountCents)).toEqual([1, 0]);
  });

  it("keeps products exact at the safe integer limit", () => {
    expect(resolveExpenseSplit(Number.MAX_SAFE_INTEGER, 1, {
      mode: "percentage", parts: [
        { ...ana, basisPoints: 5000 }, { ...bia, basisPoints: 5000 },
      ],
    }).map(part => part.amountCents)).toEqual([4503599627370496, 4503599627370495]);
  });
