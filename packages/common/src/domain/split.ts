...
export function resolveExpenseSplit(totalCents: number, installmentCount: number, split: ExpenseSplit): ResolvedAllocation[] {
  ...
  let amounts: number[];
  let parties: SplitParty[] = split.parts.map((part) =>
