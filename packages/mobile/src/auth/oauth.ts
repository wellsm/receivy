  ...
  try {
    ...
    if (provider === "apple" && Platform.OS === "ios") {
      if (!await AppleAuthentication.isAvailableAsync()) {
        throw new Error("Login Apple indisponível neste dispositivo.");
      }
      ...
      catch (error) { if (error && typeof error === "object" && "code" in error && error.code === "ERR_REQUEST_CANCELED") {
        return false;
      }

 throw error; }

      if (credential.state !== challenge.state || !credential.authorizationCode) {
    ...
    if (`${url.protocol}//${url.host}${url.pathname}` !== redirectUri) {
    ...
    if (!code) {
      throw new Error("Login não concluído. Tente novamente.");
    }
