...
import { paymentMethodText, PaymentProvider, type PaymentMethod, PixKeyType } from "@receivy/common";
...
export function FirstSharePix({ client, busy, publish }: { client: Pick<FinancialClient, "paymentMethods" | "savePaymentMethod">; busy: boolean; publish: (id: string) => Promise<void> }) {
 ...
 try { const method = await client.savePaymentMethod({ provider: PaymentProvider.Pix, kind: type, value: key });
 ...
 setItems(previous => [...previous, method]); setSelected(method.id); setKey(""); } catch (reason) { setError(reason instanceof Error ? reason.message : "Não foi possível salvar a chave."); } finally { setSaving(false); } }
  ...
  return <View className="gap-3 rounded-3xl border border-outline bg-surface p-5"><Text className="text-xl font-bold text-primary-strong">Meio de pagamento antes de compartilhar</Text><Text>Você pode manter este registro sem Pix. Para publicar, escolha a chave que ficará fixa nesta cobrança. O aviso inicial aguardará essa escolha.</Text>
    {items.map(item => <Pressable key={item.id} accessibilityRole="radio" accessibilityState={{ checked: selected === item.id }} onPress={() => setSelected(item.id)} className="min-h-12 justify-center rounded-xl border border-outline p-3"><Text>{selected === item.id ? "✓ " : ""}{paymentMethodText(item).title} · {paymentMethodText(item).value}</Text></Pressable>)}
    <Pressable accessibilityRole="button" disabled={busy || saving || !selected} onPress={() => void publish(selected)} className="min-h-12 items-center justify-center rounded-xl bg-primary"><Text className="font-bold text-on-primary">Publicar com este meio</Text></Pressable>
