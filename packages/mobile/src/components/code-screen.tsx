...
import { CODE_LENGTH, CodeBoxes } from "./code-boxes";
...
export function CodeScreen({ client = authClient, email, sentAt, onAuthenticated, onBack, now = Date.now }: CodeScreenProps) {
  ...
  return (
    <SafeAreaView className="flex-1 bg-canvas">
      <View className="flex-row items-center justify-between px-5 py-2">
        <Pressable accessibilityRole="button" accessibilityLabel="Voltar" onPress={onBack} className="min-h-12 flex-row items-center gap-2">
          <Text className="text-2xl text-ink">←</Text>
          <Text className="text-lg font-semibold text-ink">Voltar</Text>
        </Pressable>
        <View className="flex-row items-center gap-2 rounded-full bg-primary-soft/40 px-3 py-2">
          <Image source={shieldMark} style={{ width: 16, height: 16 }} />
          <Text className="text-xs font-bold text-primary-strong">Conexão Segura</Text>
        </View>
      </View>
      ...
      <KeyboardAvoidingView className="flex-1" behavior={Platform.OS === "ios" ? "padding" : undefined}>
        <ScrollView className="flex-1" contentContainerClassName="flex-grow px-5 pb-8 pt-10" keyboardShouldPersistTaps="handled">
          <View className="items-center">
            ...
            <Text className="mt-3 text-center text-base leading-6 text-muted">Enviamos um código de segurança temporário para</Text>
