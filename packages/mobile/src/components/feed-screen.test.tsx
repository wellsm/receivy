...
describe("FeedScreen", () => {
  ...
  it("filters with counters on the direction chips and keeps the latest response", async () => {
    ...
    expect(screen.getByRole("button", { name: "Feed" })).toBeOnTheScreen();
    expect(screen.getByRole("button", { name: "Cobranças" })).toBeOnTheScreen();
  ...
  it("pages with the cursor and shows the reminders badge on the bell", async () => {
    const timeline = jest
      .fn()
    ...
    await render(<FeedScreen client={{ timeline }} onOpenNotifications={onOpenNotifications} />);
