...
import { PixKeyFormScreen } from "./pix-key-form-screen";
...
describe("PixKeyFormScreen", () => {
  ...
  it("sends the unmasked key the API expects", async () => {
    ...
    await fireEvent.changeText(screen.getByLabelText("Banco (opcional)"), "Nubank");
    await fireEvent.press(screen.getByLabelText("Salvar chave Pix"));

    await waitFor(() => expect(api.savePaymentMethod).toHaveBeenCalledWith({ pixKeyType: "phone", pixKey: "+5511987654321", label: "Nubank" }));
  });
