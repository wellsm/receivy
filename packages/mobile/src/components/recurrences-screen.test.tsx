...
function client() {
  return {
    recurrences: jest.fn().mockResolvedValue({ recurrences: [] }),
    paymentMethods: jest.fn().mockResolvedValue({ paymentMethods: [] }),
    recurrenceProfile: jest
      .fn()
      .mockResolvedValue({ user: { timezone: "America/Sao_Paulo" } }),
    saveRecurrence: jest.fn(),
    transitionRecurrence: jest.fn(),
  };
}
async function newDraft() {
  const api = client();
  api.saveRecurrence.mockImplementation(async (input) => ({
    ...input,
    id: "r1",
    state: "active",
    previews: [],
...
it("retains intermediate minus during actual sequential reminder input", async () => {
  const api = await newDraft();
  const user = userEvent.setup();
  const offset = screen.getByLabelText("Dias do lembrete 1");
  await user.clear(offset);
  await user.type(offset, "-");
  expect(offset).toHaveDisplayValue("-");
  await user.type(offset, "5");
  expect(offset).toHaveDisplayValue("-5");
  await fireEvent.press(
    screen.getByRole("button", { name: "Revisar recorrência" }),
  );
  await fireEvent.press(
    screen.getByRole("button", { name: "Salvar recorrência" }),
  );
  expect(api.saveRecurrence.mock.calls[0][0].reminders[0].offsetDays).toBe(-5);
});
it("requires a new review after adding a reminder", async () => {
  const api = await newDraft();
  const user = userEvent.setup();
  await fireEvent.press(
    screen.getByRole("button", { name: "Revisar recorrência" }),
  ...
  await fireEvent.press(
    screen.getByRole("button", { name: "Adicionar lembrete" }),
  );
  expect(
    screen.queryByRole("button", { name: "Salvar recorrência" }),
  ).toBeNull();
  const offset = screen.getByLabelText("Dias do lembrete 4");
  await user.clear(offset);
  await user.type(offset, "-5");
  await fireEvent.press(
    screen.getByRole("button", { name: "Revisar recorrência" }),
  ...
  expect(api.saveRecurrence.mock.calls[0][0].reminders).toHaveLength(4);
});
it("lets an archived selected participant be replaced with an active person", async () => {
  const api = client();
  api.recurrences.mockResolvedValue({
    recurrences: [
      {
        id: "r1",
        ...
        split: {
          mode: "equal",
          parts: [{ kind: "person", personId: "archived" }],
        },
        reminders: [],
        previews: [],
        nextMaterialization: null,
  ...
  await fireEvent.press(
    await screen.findByRole("checkbox", { name: /Contato indisponível/ }),
  );
  await fireEvent.press(await screen.findByRole("checkbox", { name: "Ana" }));
  await fireEvent.press(
    screen.getByRole("button", { name: "Revisar recorrência" }),
  ...
  expect(api.saveRecurrence.mock.calls[0][0].split.parts).toEqual([
...
it("reviews exact cents and freezes uncertain recurrence retries", async () => {
  ...
  expect(api.saveRecurrence.mock.calls[1]).toEqual(
    api.saveRecurrence.mock.calls[0],
  );
...
it("ends only after confirmation and leaves no edit or payment action", async () => {
  ...
  api.recurrences.mockResolvedValue({ recurrences: [rule] });
  api.transitionRecurrence.mockResolvedValue({ ...rule, state: "ended" });
  await render(<RecurrencesScreen client={api} people={people} />);
  await fireEvent.press(
