...
export function createFinancialClient({ authenticatedFetch, publicWebBaseUrl }: Options) {
  async function request<T>(path: string, init?: RequestInit, fallback = "Não foi possível acessar seus registros financeiros."): Promise<T> {
    ...
    if (!response.ok) {
      throw new FinancialRequestError(await message(response, fallback), response.status);
    }
  ...
  return {
    ...
    /** The whole month in one answer; the feed filters and sums it locally. */
    charges(month: string) { return request<ListCharge>(`charges?month=${encodeURIComponent(month)}`); },
 ...
 try { base = new URL(publicWebBaseUrl); } catch { throw new Error("EXPO_PUBLIC_WEB_URL precisa ser uma URL web válida."); }
