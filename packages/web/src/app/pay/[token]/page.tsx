...
export default async function PublicChargePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="public-charge"><div className="public-brand">Receivy</div><section>
    <span className={`state-label ${charge.state}`}>{charge.state === "pending" ? "Pendente" : charge.state === "paid" ? "Pago" : "Cancelado"}</span>
    <h1>{charge.description}</h1><p>Cobrança de {charge.creditorFirstName}</p><strong className="public-amount">{formatMoney(charge.amount)}</strong>
    <dl><div><dt>Vencimento</dt><dd>{new Intl.DateTimeFormat("pt-BR", { dateStyle: "long", timeZone: "UTC" }).format(new Date(`${charge.dueDate}T00:00:00Z`))}</dd></div></dl>
    {charge.pix && charge.state === "pending" && <div className="public-pix"><h2>Chave Pix</h2><code>{charge.pix.key}</code><PublicPixCopy pixKey={charge.pix.key} /><p>Confira o nome do destinatário no seu banco antes de transferir.</p></div>}
    <ProofPanel base={`/api/public-proof/${encodeURIComponent(token)}`} state={charge.state} publicView uploadsEnabled={charge.uploadsEnabled} />
  </section></main>;
}
