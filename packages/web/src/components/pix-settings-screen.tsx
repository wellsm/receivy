...
export function PixSettingsScreen({ returnTo }: { returnTo?: string }) {
  const router = useRouter();
  const [items, setItems] = useState<PaymentMethod[]>([]); const [type, setType] = useState<PixKeyType>("email"); const [key, setKey] = useState(""); const [label, setLabel] = useState(""); const [editing, setEditing] = useState<string | null>(null); const [busy, setBusy] = useState(false); const [error, setError] = useState(""); const [notice, setNotice] = useState("");
