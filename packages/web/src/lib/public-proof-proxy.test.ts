...
it("only permits a configured exact upload origin in CSP", () => {
  ...
  for (const value of [undefined, "*", "https:", "https://user:pass@evil.test", "https://good.test/path", "http://evil.test", "https://good.test; script-src *"]) {
    expect(proofUploadOrigin(value)).toBeNull();
  }
});
